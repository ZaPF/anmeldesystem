...
class Winter20Registration(FlaskForm):
    ...
    def __init__(self, **kwargs):
        ...
        self.immatrikuliert.validators = [
            ImmatrikulationsValidator(self.immatrikuliert)
        ]
        self.immatrikulationsbescheinigung.validators = [
            ImmatrikulationsValidator2(self.immatrikulationsbescheinigung)
        ]
    ...
    immatrikulationsbescheinigung = RadioField(
    ...
    allergien = StringField(
        "Hast du irgendwelche Allergien oder andere Essenseinschränkungen?"
    )
    ...
    eigene_unterkunft = BooleanField(
        "Ich könnte mich selber um eine Unterkunft kümmern, falls ihr mir keinen Schlafplatz anbieten könnt."
    )
    ...
    hoodie = SelectField(
        "Ich möchte gerne eine Sweatjacke für 25 Euro bestellen.",
        choices=HOODIE_CHOICES,
    )
    ...
    zaepfchen = SelectField(
        ...
        choices=[
            ...
            (
                "ja",
                "Ja, aber ich möchte keinen Mentor oder bringe meinen eigenen Mentor mit.",
            ),
            ("nein", "Nein"),
        ],
    )
    ...
    vertrauensperson = BooleanField(
        "Ich möchte mich als Vertrauensperson zur Wahl stellen."
    )
    stream = RadioField(
        "Wärst du damit einverstanden, dass die Plenen Live ins Internet gestreamt werden?*",
        [validators.InputRequired()],
        choices=[
            ("nein", "Nein"),
    ...
    langzeithelfikon = BooleanField(
        "Ich bin bereit, im laufe der ZaPF mehr als 12 Stunden als Helfikon tätig zu sein."
    )
    ...
    #    ])'''
    datenschutz = BooleanField(
        "Ich habe die Datenschutzerklärung gelesen und bin mit der darin beschriebenen Verarbeitung meiner Daten einverstanden.*",
        [validators.InputRequired()],
    )
    corona = BooleanField(
        "Ich bin mir bewusst, dass vor der Tagung eine Hausordnung inklusive Hygienemaßnahmen veröffentlicht wird und werde mich an diese Regeln halten.",
        [validators.InputRequired()],
    )
    korrekt = BooleanField(
        "Die oben angegebenen Daten sind meine eigenen, korrekt und aktuell. Sollten sie sich bis zum Ende der ZaPF ändern, werde ich die ausrichtende Fachschaft darüber informieren.",
        [validators.InputRequired()],
    )
...
def index():
    ...
    if "me" not in session:
        return render_template(
            ...
            registration_close=(
                current_app.config["REGISTRATION_SOFT_CLOSE"]
                .astimezone(app.config["TIMEZONE"])
                .strftime("%d.%m. um %H:%M Uhr %Z"),
                current_app.config["REGISTRATION_HARD_CLOSE"]
                .astimezone(app.config["TIMEZONE"])
                .strftime("%d.%m. um %H:%M Uhr %Z"),
            ),
        )
    ...
    if unis._resp.code != 200:
        return redirect(url_for("oauth_client.login"))
    form.uni.choices = [("", "Bitte Auswählen")] + sorted(
        unis.data.items(), key=lambda uniEntry: int(uniEntry[0])
    )
