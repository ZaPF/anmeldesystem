...
def post_registrations(token, priorities):
    ...
    r = s.post(
        current_app.config["ZAPFAUTH_BASE_URL"] + "priorities",
        json={"confirmed": priorities},
    )
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            form.password.errors.append(
                "Der angegebene Uni-Token ist falsch. Euer Uni-Token sollte Euch in der Einladung zugeschickt worden sein."
            )
        except:
            raise
        else:
...
def check_if_closed(f):
    ...
    def decorated_function(*args, **kwargs):
        if (
            ...
            and not current_app.config["REGISTRATION_FORCE_PRIORITIES OPEN"]
        ):
...
def priorities_index():
    ...
    confirmed = [
        ...
        if registration["priority"] is not None
    ]
    unconfirmed = [
        registration
        for registration in registrations["registrations"]
...
def del_mascot(id):
    ...
    r = s.post(
        current_app.config["ZAPFAUTH_BASE_URL"] + "mascots/delete", json={"id": id}
    )
...
def _generate_post_payload(priorities_dict):
    ...
    return sorted(
        [
            k
            for k in priorities_dict
            if priorities_dict[k] is not None and priorities_dict[k] != -1
        ],
