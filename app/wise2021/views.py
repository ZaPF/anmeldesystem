...
###### Essen ######
    ...
    essensmenge = SelectField('Wieviele Brötchen würdest du morgens essen', choices=[
	('eins', 'Eins'),
	('zwei', 'Zwei'),
	('drei', 'Drei'),
	('mehr', 'Mehr!'),
...
#        ('egal', 'Egal'),
    ...
    getraenkewunsch = StringField('Unverbindliches Lieblingsgetränk für eine hypothetische Party:') ## drin lassen?
...
#### Reiseinfos ####
    ...
    anreise_verkehr = SelectField('Verkehrsmittel deiner Wahl', choices=[
        ('bus', 'Fernbus'),
        ('bahn', 'Zug'),
        ('auto', 'Auto'),
#        ('flug', 'Flugzeug'),
...
##### Standorte ######
	...
	barrierefreiheit = BooleanField('Ich habe spezifische Ansprüche an Barrierefreiheit.')

	standort #hier abfrage zu standorten

	impfstatus = SelectField('Impfstatus', choices=[
	('keinfach', 'ungeimpft'),
	('einfach', 'einfach geimpft'),
	('zweifach', 'zweifach geimpft'),
	('schutz', 'vollständig geschützt (zwei Wochen nach der zweiten Impfung)'),

	impfstatus2 = BooleanField('Ich werde meinen Impfausweis dabei haben und bei Bedarf vorzeigen', [validators.InputRequired()])

	partner = StringField('Wunschpartner')

	partner2 = BooleanField('Ich komme nur in Präsenz, wenn ich mit der oben angegebenen Person zusammen kommen kann.')	

  	schlafen = SelectField('Für die Unterkunft:', choices=[
	('toiletten', 'Ich möchte keine binären Duschen und Toiletten verwenden'),
       	('doppelbett1','Ich bin einverstanden mit einer beliebigen Person in einem Doppelbett zu schlafen'),
       	('doppelbett2','Ich bin einverstanden mit einer bestimmten Person in einem Doppelbett zu schlafen'),
       ('groesse','Ich bin klein'),
       ])

	unterbringung = SelectField('Private Unterbringung in:', choices=[
	## hier die Standorte
	])
...
#    halle = BooleanField('Ich habe die Hallenordnung (siehe <a href="https://bonn.zapf.in/index.php/hallenordnung/">Website</a>) gelesen und verstanden und werde mich daran halten.', [validators.InputRequired()])
    minderjaehrig = SelectField('Ich bin zum Zeitpunkt der ZaPF:', choices=[
